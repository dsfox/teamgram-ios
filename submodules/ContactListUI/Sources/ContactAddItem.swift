...
public class ContactsAddItem: ListViewItem {
    ...
    /// What the row says; nil is upstream's "Add <number>" (#164).
    let title: String?
    let action: () -> Void
    
    public let header: ListViewItemHeader?
    
    public init(context: AccountContext, theme: PresentationTheme, strings: PresentationStrings, phoneNumber: String, title: String? = nil, header: ListViewItemHeader?, action: @escaping () -> Void) {
...
class ContactsAddItemNode: ListViewItemNode {
    ...
    func asyncLayout() -> (_ item: ContactsAddItem, _ params: ListViewItemLayoutParams, _ first: Bool, _ last: Bool, _ firstWithHeader: Bool) -> (ListViewItemNodeLayout, (Bool) -> Void) {
        ...
        return { [weak self] item, params, first, last, firstWithHeader in
            ...
            let titleText = item.title ?? item.strings.Contacts_AddPhoneNumber(formatPhoneNumber(context: item.context, number: item.phoneNumber)).string
            let titleAttributedString = NSAttributedString(string: titleText, font: titleFont, textColor: item.theme.list.itemAccentColor)
